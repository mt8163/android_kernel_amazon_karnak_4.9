...
#else
	/*12.poll CONNSYS CHIP ID until 8163 is returned 0x18070008 32'h6755 */
	while (retry-- > 0) {
...
static INT32 consys_hw_vcn18_ctrl(MTK_WCN_BOOL enable)
{
	...
	KERNEL_upmu_set_vcn_1v8_lp_mode_set(0);
#else
...
#if CONSYS_PMIC_CTRL_UPMU
		KERNEL_upmu_set_vcn28_on_ctrl(1);
#else
		KERNEL_pmic_set_register_value(PMIC_RG_VCN28_ON_CTRL, 1);
#endif
	} else {
#if CONSYS_PMIC_CTRL_UPMU
		KERNEL_upmu_set_vcn28_on_ctrl(0);
#else
	...
	if (enable) {
		...
		if (reg_VCN28)
			if (regulator_disable(reg_VCN28))
				WMT_PLAT_PR_ERR("WMT do VCN28 PMIC off fail!\n");
#endif
...
static INT32 consys_hw_bt_vcn33_ctrl(UINT32 enable)
{
		...
		if (1 == gBtWifiV33.counter) {
			...
			if (reg_VCN33_BT)
				if (regulator_enable(reg_VCN33_BT))
					WMT_PLAT_PR_ERR("WMT do BT/WIFI off fail!\n");
#endif
			KERNEL_upmu_set_vcn33_on_ctrl_bt(1);
#endif
...
#if CONSYS_PMIC_CTRL_ENABLE
			KERNEL_upmu_set_vcn33_on_ctrl_bt(0);
#if defined(CONFIG_MTK_PMIC_LEGACY)
	...
	if (enable) {
		...
		if (reg_VCN33_BT)
			if (regulator_disable(reg_VCN33_BT))
				WMT_PLAT_PR_ERR("WMT do BT PMIC off fail!\n");
#endif
#endif
	...
	if (enable) {
...
#endif
#if CONSYS_PMIC_CTRL_UPMU
		KERNEL_upmu_set_vcn33_on_ctrl_wifi(1);
#else
...
#if CONSYS_PMIC_CTRL_ENABLE
#if CONSYS_PMIC_CTRL_UPMU
		KERNEL_upmu_set_vcn33_on_ctrl_wifi(0);
#else
		...
		if (reg_VCN33_WIFI)
			if (regulator_disable(reg_VCN33_WIFI))
				WMT_PLAT_PR_ERR("WMT do WIFI PMIC off fail!\n");
#endif
